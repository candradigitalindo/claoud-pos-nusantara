<template>
  <div class="space-y-5">
    <div class="flex flex-wrap items-center justify-between gap-2">
      <h1 class="text-lg sm:text-xl font-bold text-gray-900">Pengadaan Jasa</h1>
    ...
    <AppCard :padding="false">
      ...
      <div class="sm:hidden">
        ...
        <div v-else-if="!requests.length" class="p-6 text-center text-sm text-gray-400">Belum ada pengajuan jasa.</div>
      ...
      <AppTable class="hidden sm:block" :columns="COLUMNS" :rows="requests" :loading="loading" emptyText="Belum ada pengajuan jasa.">
...
function emptyForm() {
  return {
    ...
    items: [{ name: '', items: [{ name: '', qty: 1, unit: 'paket', hps_price: 0, hps_source: '', rab_item_id: '' }] }],
    notes: '',
  }
}
function addItem() {
  form.value.items.push({ name: '', items: [{ name: '', qty: 1, unit: 'paket', hps_price: 0, hps_source: '', rab_item_id: '' }] })
}
function removeItem(i) { if (form.value.items.length > 1) form.value.items.splice(i, 1) }
function addSubItem(i) { form.value.items[i].items.push({ name: '', qty: 1, unit: 'paket', hps_price: 0, hps_source: '', rab_item_id: '' }) }
